...
// ----------------------------------------------------------------------------
	 ...
	 : size_x(size_x), size_y(size_y)
 {
	 ...
	 clear();
        ...
        void Buffer::set(unsigned int pos_x, unsigned int pos_y, std::string s){		//confused here
			for (int i=0, x=pos_x, y=pos_y; i < s.length(); i++){						//this is from github buffer solution set
				if (s[i] == '\n'){
					y++;
					x=pos_x;
				} 
        ...
        /**
         * Output the contents of the buffer to `cout`.		//nested for loops
         */
		...
		void Buffer::drawShape(int x, int y, const Shape &s) {
			int tx = x;
			int ty = y;
			for (int i = 0; i < s.type_art, lenght(); i++) {
				if (s.type_art[i] == '\n') {
					y++;
					i = x;
				}
				set(tx++, ty, s.type_art[i]);
				tx++;
			}
		}
